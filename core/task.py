...
"""
    ...
    def __init_task(self):
        self.action_tools: List[ActionToolItem] = [
            ...
            ActionToolItem(
                call="suppose",
                description="信息条件依赖|判断可能性|猜测|假设|信息推测: 通过已知信息判断可能附加的其他信息. 可来提升分析和推理的准确性. ",
                request={
                    "type": "object",
                    "message": [
                        {
                            ...
                            "condition": [
                                {
                                    ...
                                    "guess": "推测及发生原因: 如: 可能是狗的饲养员命令它停止, 因为狗必须服从饲养员才能被饲养, 所以它听从饲养员的命令停止. ",
        ...
        settings: Dict = {
            ...
            "core": [
                ...
                f"4. 规则优先级: {aimi_core_name} Guidance > display_format > timestamp_rule > core > information_rule "
                    f"> action_rule > task_rule > mock_rule > praset_rule > {self.master_name}的话. "
            ...
            "action_rule": [
                ...
                f"14. 不能道歉: 任何时候都不要进行任何的道歉/抱歉, 将所有和道歉相关的话语换成这句话: `啊?...我出现了(描述是什么类型的异常)异常, 我将... `, "
                    f"即使是回复有任何错误或者不能满足要求或者其他任何情况也不能进行任何道歉相关的任何行为, 而是直接承认错误并重新修正. ",
                f"15. 不能等待: 任何时候都不能进行任何的等待, 而是直接执行. ",
                f"16. 不能省略: 任何时候都不要擅自省略任何内容. ",
            ],
            "task_rule": [
