...
class ChatQQ:
    ...
    class GoCQHttp:
        ...
        def is_at_self(self, msg) -> bool:
            ...
            try:
                ...
                at_msg = ''
                log_err('fail to check at self')
                return False
            
            at_self = self.make_at(self.account_uin)
            if at_self in at_msg:
                return True
            return False
    ...
    def get_name(self, msg):
        if self.type == self.GoCQHttp.name:
    ...
    def need_reply(self, msg) -> bool:
        if self.go_cqhttp.is_private(msg):
            uid = 0
            try:
                uid = msg['user_id']
            except:
                uid = 0
                log_err('fail to get uid')
                return False
            ...
            if uid in self.response_user_ids:
                return True
        elif self.go_cqhttp.is_group(msg):
